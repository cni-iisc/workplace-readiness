...
This file preserves the previous README content for project and domain context.
The current deployment entry point is [Deploy On Ubuntu VM](DEPLOY_ON_VM.md).
...
4. Create a symbolic link to sites-enabled folder with the following command: `sudo ln -s /etc/nginx/sites-available/example.com /etc/nginx/sites-enabled/`
...
2. The reference implementation reads fields from SAMPLE_ENV file while running the server code. Fields like SENDER_EMAIL, SENDER_PASSWORD, CAPTCHA_PRIVATE, DB_JSON, DB_FEEDBACK, HTTP_ORIGIN can be configured in this file. Once this field are configured we can start running our services.
